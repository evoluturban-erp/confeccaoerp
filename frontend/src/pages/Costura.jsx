...
const FASES_COSTURA  = ['Costura','Acabamento','Revisão','Expedição'];
...
function GradeInput({ cores, tamanhos, grade, onChange, loading }) {
  ...
  return (
    <div style={{ overflowX:'auto' }}>
      <p style={{ margin:'0 0 8px', fontSize:'.75rem', color:'#6b7280' }}>Sugestão baseada na grade planejada. Ajuste conforme o corte real.</p>
...
export default function Costura() {
  ...
  const { data: fornecedores = [] } = useApiQuery(['forn-costura'], () => api.get('/fornecedores?setor=Costura'));
  const { data: ordens = [] }       = useApiQuery(['ordens-costura'], () => api.get('/ordens'));
  const { data: enviosAPI = [] }    = useApiQuery(['envios-faccao'], () => api.get('/coletas?tipo=envio_faccao'));
  const { data: opRefs = [], isFetching: loadingRefs } = useApiQuery(
    ['op-refs-costura', form.op?.id],
  ...
  const opsDisp = ordens.filter(o => FASES_COSTURA.includes(o.fase_atual));
      ...
      {faccoes.length > 0 && (
        <div style={{ background:'#fff', borderRadius:12, padding:'14px 18px', boxShadow:'0 1px 4px rgba(0,0,0,.07)' }}>
          ...
          <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
            {faccoes.map(f => {
              const ativos = envios.filter(e=>e.faccao_nome===f.nome&&e.status==='enviado').length;
              return (
                <div key={f.id} style={{ padding:'10px 16px', borderRadius:10, border:`1px solid ${ativos?'#fcd34d':'#e5e7eb'}`, background:ativos?'#fef3c7':'#f9fafb', display:'flex', alignItems:'center', gap:10, minWidth:180 }}>
                  ...
                  <div>
                    <p style={{ margin:0, fontSize:'.82rem', fontWeight:700, color:'#111827' }}>{f.nome}</p>
        ...
        <ModalOverlay onClose={()=>setModal(null)}>
          <div style={{ background:'#fff', borderRadius:14, width:'100%', maxWidth:700, maxHeight:'90vh', overflow:'hidden', display:'flex', flexDirection:'column', boxShadow:'0 20px 60px rgba(0,0,0,.2)' }}>
              ...
              {step===0 && (
                <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
                  <div style={{ display:'flex', flexDirection:'column', gap:4 }}>
                    <label style={{ fontSize:'.78rem', fontWeight:600, color:'#374151' }}>Facção *</label>
                    <select value={form.faccao?.id||''} onChange={e=>setForm(f=>({...f,faccao:faccoes.find(x=>String(x.id)===e.target.value)||null}))}
                      ...
                      {faccoes.map(x=><option key={x.id} value={x.id}>{x.nome}</option>)}
                    </select>
                    {faccoes.length===0 && <p style={{ margin:0, fontSize:'.72rem', color:'#9ca3af' }}>Nenhuma facção cadastrada. Acesse Fornecedores → setor Costura.</p>}
                  </div>
                  <div style={{ display:'flex', flexDirection:'column', gap:4 }}>
                    ...
                    {opsDisp.length===0 && <p style={{ margin:0, fontSize:'.72rem', color:'#9ca3af' }}>Nenhuma OP na fase de Costura. Avance OPs pelo módulo de Corte.</p>}
                    {form.op && (
                      <div style={{ padding:'8px 12px', borderRadius:8, background:'#f0fdf4', border:'1px solid #bbf7d0', fontSize:'.78rem', color:'#374151', display:'grid', gridTemplateColumns:'1fr 1fr', gap:4 }}>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
                  <div style={{ padding:'14px 16px', borderRadius:10, background:'#f0fdf4', border:'1px solid #bbf7d0' }}>
                    ...
                    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, fontSize:'.8rem', color:'#374151' }}>
                      <div><span style={{ color:'#6b7280' }}>Facção: </span><b>{form.faccao?.nome||'—'}</b></div>
                      <div><span style={{ color:'#6b7280' }}>OP: </span><b>{form.op?.numero||'—'}</b></div>
                      <div><span style={{ color:'#6b7280' }}>Cliente: </span>{form.op?.cliente_nome||'—'}</div>
                      <div><span style={{ color:'#6b7280' }}>Total peças: </span><b style={{ color:'#16a34a' }}>{totalG(form.grade)}</b></div>
                    ...
                    <div>
                      ...
                      <div style={{ display:'flex', flexWrap:'wrap', gap:8 }}>
                        {opCores.map(cor => {
                          ...
                          return tot > 0 ? (
                            <div key={cor} style={{ padding:'6px 14px', borderRadius:8, background:'#f0fdf4', border:'1px solid #bbf7d0', fontSize:'.8rem' }}>
                              <b>{cor}:</b> {tot} pcs
                            </div>
                          ) : null;
                        })}
            ...
            <div style={{ padding:'12px 22px', borderTop:'1px solid #f1f5f9', display:'flex', justifyContent:'space-between' }}>
              ...
              {step<2
                ? <button onClick={()=>setStep(s=>s+1)} style={{ padding:'7px 22px', borderRadius:8, background:'#16a34a', color:'#fff', border:'none', fontSize:'.85rem', fontWeight:600, cursor:'pointer' }}>Próximo →</button>
                : <button onClick={confirmarEnvio} disabled={saving} style={{ padding:'7px 22px', borderRadius:8, background:saving?'#9ca3af':'#16a34a', color:'#fff', border:'none', fontSize:'.85rem', fontWeight:600, cursor:saving?'not-allowed':'pointer' }}>
      ...
      {modal==='retorno' && envioSel && (
        <ModalOverlay onClose={()=>setModal(null)}>
          <div style={{ background:'#fff', borderRadius:14, width:'100%', maxWidth:720, maxHeight:'90vh', overflow:'hidden', display:'flex', flexDirection:'column', boxShadow:'0 20px 60px rgba(0,0,0,.2)' }}>
            <div style={{ padding:'14px 22px', borderBottom:'1px solid #f1f5f9', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
              <div>
                <h3 style={{ margin:'0 0 2px', fontSize:'1rem', fontWeight:700 }}>Conferência de Retorno — {envioSel.faccao_nome}</h3>
                    ...
                    <tbody>
                      {allCores.flatMap(cor => [
                        ...
                        <tr key={`r-${cor}`}>
                          ...
                          <td style={{ padding:'6px 12px', border:'1px solid #e5e7eb', textAlign:'center', fontWeight:700, color:'#16a34a' }}>{envioTams.reduce((s,t)=>s+(Number(retorno[cor]?.[t])||0),0)}</td>
                        </tr>
                      ])}
                    </tbody>
                  </table>
